...
{
    echo "Getting WebLogic cluster address based on admin URL supplied"
    restArgs=" -v --insecure --user ${WLS_USER}:${WLS_PASSWORD} -H X-Requested-By:MyClient -H Accept:application/json -H Content-Type:application/json"
    echo "curl $restArgs -X GET ${WLS_REST_URL}/domainRuntime/serverRuntimes?fields=defaultURL"
    
    echo "Attempting to access the WebLogic REST URL : ${WLS_REST_URL}"
    HTTP_STATUS=$( curl $restArgs -o /dev/null -s -w "%{http_code}\n" -X GET ${WLS_REST_URL}/domainRuntime/serverRuntimes?fields=defaultURL )
    if [ $HTTP_STATUS != 200 ]
    then
    	echo_stderr "Unable to access ${WLS_REST_URL}. Please check supplied URL and credentials"
    	exit 1
    fi
    ...
    if [ -z "$wlsClusterAddress" ] 
    then
       echo_stderr "Received empty WebLogic cluster address. Please check the supplied admin url and credentials"
       exit 1
    fi
    
    export WLS_CLUSTER_ADDRESS=${wlsClusterAddress::-1}
    echo "WebLogic Cluster Address URL: $WLS_CLUSTER_ADDRESS"	
    
    if [ -z "$WLS_CLUSTER_ADDRESS" ] 
    then
       echo_stderr "Received empty WebLogic cluster address. Please check the supplied admin url and credentials"
       exit 1
    fi
